...
public class AdjacencyList {
	...
	public AdjacencyList(){
		conf = new JobConf(AdjacencyList.class);
		conf.addResource("p3-default.xml");
	}
	
	public AdjacencyList(JobConf conf){
		this.conf = conf;
	}
	...
	*******************************************/
	...
	implements Mapper<LongWritable, Text, Text, Text>{
		...
		public void map
			...
			String sip = tokens[key_field];
			
			output.collect(new Text(sip), value);	
		}
	}
	...
	private JobConf getTopNJobConf(String jobName, Path inFilePath, Path outFilePath){
		...
		JobConf conf = new JobConf(AdjacencyList.class);
	...
	*******************************************/
	...
	implements Mapper<LongWritable, BytesWritable, Text, Text>{
		...
		public void map
			...
			byte[] ip_ver = {0x00};	
			byte[] sip = new byte[PcapRec.LEN_IPADDR];
			byte[] dip = new byte[PcapRec.LEN_IPADDR];
			...
			if(BinaryUtils.byteToInt(eth_type) == PcapRec.IP_PROTO) {			
				...
				if((BinaryUtils.byteToInt(ip_ver) & PcapRec.IPV4) == PcapRec.IPV4){
					System.arraycopy(value_bytes, PcapRec.POS_SIP, sip, 0, sip.length);	
					System.arraycopy(value_bytes, PcapRec.POS_DIP, dip, 0, dip.length);
					
					strSIP = CommonData.longTostrIp(Bytes.toLong(sip));
					strDIP = CommonData.longTostrIp(Bytes.toLong(dip));
					
					output.collect(new Text(strSIP), new Text(strDIP));		
//					output.collect(new Text(strDIP), new Text(strSIP));	
				}				
			}
		}
	}
	...
	implements Reducer<Text, Text, Text, Text> {
        public void reduce(Text key, Iterator<Text> value,
			...
			String previous = "-1.-1.-1.-1";

           while(value.hasNext()){ 
        	   node = value.next().toString();
...
//        conf.setNumReduceTasks(10);       
	...
	try {
        ...
        JobConf countJobconf = getCountUpJobConf("AdjacencyList", inputDir, outputDir);        
	...
	public static void main(String[] args) throws Exception{
		...
		AdjacencyList cu = new AdjacencyList();
