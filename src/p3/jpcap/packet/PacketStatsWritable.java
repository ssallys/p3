...
import p3.hadoop.common.packet.NewPcapRec;
...
*/
	...
	public boolean parse(byte[] pdata){
		...
		// tcp statistics
		if(eb.GetByte(NewPcapRec.POS_IPHDR+NewPcapRec.POS_IP_PROTO) == IPPacket.IPPROTO_TCP){
		
			this.tcp = true;
			int tcpHLEN = (eb.GetByte(tcpHPOS+NewPcapRec.POS_TCP_OFFSET)>>4)* 4;
			
			int unique_bytes = totLEN - ipHLEN - tcpHLEN ;
			
			this.setFirst_time(Bytes.toLong(BinaryUtils.flipBO(eb.GetBytes(0, 4),4))*1000000
					+ Bytes.toLong(BinaryUtils.flipBO(eb.GetBytes(4, 4),4)));
			this.setLast_time(Bytes.toLong(BinaryUtils.flipBO(eb.GetBytes(0, 4),4))*1000000
					+ Bytes.toLong(BinaryUtils.flipBO(eb.GetBytes(4, 4),4)));				

			this.setUnique_bytes(unique_bytes > 0 ? unique_bytes : 0 );	
			
			if(capLen+PCAPHEADER_LEN < tcpHPOS+tcpHLEN)
				return true;	
			
			this.setSrc_port(Bytes.toInt(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_SPORT,2)));
			this.setDst_port (Bytes.toInt(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_DPORT,2)));
			this.setPackets(1);		

			this.setSyn((((eb.GetByte(tcpHPOS+NewPcapRec.POS_TCP_CTRL)>>1)& 0x01)==0x01)?1:0);
			this.setFin(((eb.GetByte(tcpHPOS+NewPcapRec.POS_TCP_CTRL)& 0x01)==0x01)?1:0);
			this.setAck((((eb.GetByte(tcpHPOS+NewPcapRec.POS_TCP_CTRL)>>4) & 0x01)==0x01)?1:0);
			this.setAcknum(Bytes.toLong(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_ACK, 4)));

			this.setRst((((eb.GetByte(tcpHPOS+NewPcapRec.POS_TCP_CTRL)>>2)& 0x01)==0x01)?1:0);					
			this.setPure_ack((((eb.GetByte(tcpHPOS+NewPcapRec.POS_TCP_CTRL)>>4)& 0x01)==0x01)&& unique_bytes < 1 ? 1:0);
			this.setData_pkts((long)(unique_bytes>0?1:0));
			this.setData_bytes(unique_bytes); //include retransmissions

			boolean exitOpt = false;
			for(int opt=20 ; opt<tcpHLEN;){
				if (!exitOpt) break;
				switch(eb.GetByte(tcpHPOS+opt)){
				case 0x00:
					opt++;
					exitOpt = true;
					break;
				case 0x01:
					...
					break;
				case 0x02:
					opt+=Bytes.toInt(eb.GetBytes(tcpHPOS+opt+1,1));;
					this.setMss(Bytes.toInt(eb.GetBytes(tcpHPOS+opt+2,
							Bytes.toInt(eb.GetBytes(tcpHPOS+opt+1,1))- 2)));
					break;	
				case 0x03:
					opt+=Bytes.toInt(eb.GetBytes(tcpHPOS+opt+1,1));
					this.setWs(true);
					this.setWin_scale(Bytes.toInt(eb.GetBytes(tcpHPOS+opt+2,
							Bytes.toInt(eb.GetBytes(tcpHPOS+opt+1,1))- 2)));
					break;
				case 0x08:
					opt+=Bytes.toInt(eb.GetBytes(tcpHPOS+opt+1,1));;
			...
			seq_min = Bytes.toLong(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_SEQ, 4));
			seq_max = Bytes.toLong(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_SEQ, 4));

			this.win_min = Bytes.toInt(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_WIN,2));
			this.win_max = Bytes.toInt(eb.GetBytes(tcpHPOS+NewPcapRec.POS_TCP_WIN,2));
			this.setCnt_zerowin(this.win_min == 0?1:0);
			
			this.setCwin_max(0);
			this.setCwin_min(0);
			this.setInitial_cwin(1);
			this.setTot_rtt(0);
			this.setRtt_min(0);
			this.setRtt_max(0);
			
			this.setTtl(Bytes.toInt(eb.GetBytes(NewPcapRec.POS_IPHDR+NewPcapRec.POS_IP_TTL,1)));
			this.setTtl_max(this.getTtl());
			this.setTtl_min(this.getTtl());		
			
			this.setReordering(0);
			this.setNet_dup(0);
			this.setUnknown(0);

			this.setRexmit_pkts(0);
			this.setRexmit_bytes(0);
			this.setOut_seq_pkts(0);
			
			int appHPOS = tcpHPOS + tcpHLEN;

			// application type & sub type
			if(capLen-appHPOS>0)
				this.setPayload(eb.GetBytes(appHPOS, capLen-appHPOS));
			this.setApp_type(TCP_APP_PROTO.UNKNOWN);
						
		}/*
		else if(eb.GetByte(NewPcapRec.POS_IPHDR+NewPcapRec.POS_IP_PROTO) == IPPacket.IPPROTO_UDP){
			...
			this.setPackets(1);
			int udpHLEN = 8;
			int unique_bytes = totLEN - ipHLEN - udpHLEN ;
			
			this.setData_pkts (unique_bytes>0 ? 1:0);
			this.setData_bytes(unique_bytes); //include retransmissions	
			
			this.setFirst_time(Bytes.toLong(BinaryUtils.flipBO(eb.GetBytes(0, 4),4))*1000000
				+ Bytes.toLong(BinaryUtils.flipBO(eb.GetBytes(4, 4),4)));
			this.setLast_time(this.getFirst_time());	
					
			if(capLen+PCAPHEADER_LEN < udpHPOS+udpHLEN)
				return true;	
					
			this.setSrc_port(Bytes.toInt(eb.GetBytes(udpHPOS+NewPcapRec.POS_UDP_SPORT, 2)));
			this.setDst_port(Bytes.toInt(eb.GetBytes(udpHPOS+NewPcapRec.POS_UDP_DPORT, 2)));
			
			int appHPOS = udpHPOS + udpHLEN;
			
			// check app type & direction
			this.setApp_type(TCP_APP_PROTO.UNKNOWN);
			// DNS
			if(dst_port == 53 || src_port == 53){
				app_type = UDP_APP_TYPE.DNS;			
			}
			// NetBOIS Name Service
			else if(dst_port == 137 && src_port == 137){
				app_type = UDP_APP_TYPE.NBNS;
			} 
			
			this.setDirection(PacketStatsWritable.NONE);
			if (app_type == UDP_APP_TYPE.DNS || app_type == UDP_APP_TYPE.NBNS){
				if (eb.GetShort(appHPOS+NewPcapRec.POS_NS_QUERY)== 1 
						&& eb.GetShort(appHPOS+NewPcapRec.POS_NS_ANSWER)== 0)
					this.setDirection(C2S);
				else if (eb.GetShort(appHPOS+NewPcapRec.POS_NS_QUERY)== 1 
						&& eb.GetShort(appHPOS+NewPcapRec.POS_NS_ANSWER)== 1)	
					this.setDirection(S2C);
			}
			// application type & sub type
			if(capLen-appHPOS>0)
				this.setPayload(eb.GetBytes(appHPOS, capLen-appHPOS));
	...
	public static final class STATE_TYPE
	{ 
		public static final int UNKNOWN_TYPE = 0 ;
		public static final int RTSP_COMMAND = 1 ;
		public static final int RTSP_RESPONSE = 2 ;
		public static final int HTTP_COMMAND = 3 ;
		public static final int HTTP_RESPONSE = 4 ;
		public static final int MSN_VER_C2S =5;
		public static final int MSN_VER_S2C =6;
		public static final int MSN_USR_C2S =7;
		public static final int MSN_USR_S2C =8;
		public static final int MSN_ANS_COMMAND =9;
		public static final int MSN_IRO_COMMAND =10;
		public static final int YMSGP =11;
		public static final int XMPP =12;
		public static final int SMTP_OPENING =13;
		public static final int POP3_OPENING =14;
		public static final int IMAP_OPENING =15;
		public static final int IMAP_COMMAND =16;
		public static final int SSL_HANDSHAKE =17;
		public static final int SSH_SERVER =18;
		public static final int RTMP_HANDSHAKE =19;
		public static final int IGNORE_FURTHER_PACKETS = 20;
	}
	
	/* Application layer protocol - Upper Level protocols (bitmask)*/
	public static final class TCP_APP_PROTO{
		public static final int UNKNOWN = 0;
		public static final int HTTP =   1;
		public static final int RTSP =   2;
		public static final int RTP =   4;
		public static final int ICY =   8;
		public static final int RTCP =  16;
		public static final int MSN =   32;
		public static final int YMSG =  64;
		public static final int XMPP =  128;
		public static final int P2P =  256;
		public static final int SKYPE = 512;
		public static final int SMTP = 1024;
		public static final int POP3 = 2048;
		public static final int IMAP = 4096;
		public static final int SSL =  8192;
		public static final int OBF = 16384;
		public static final int SSH = 32768;
		public static final int RTMP = 65536;
		public static final int MSE = 131072;
	}
	
	public static final class HTTP_CONTENT
	{
		public static final int HTTP_GET =1;			/*0 - Unclassified GET command 		*/
		public static final int HTTP_POST =2;			/*1 - Unclassified POST command 		*/
		public static final int HTTP_MSN =3;			/*2 - MSN Chat command tunneled over HTTP (POST) */
		public static final int HTTP_RTMPT =4;			/*3 - RTMPT - RTMP over HTTP Tunnel (POST) 	*/
		public static final int HTTP_YOUTUBE_VIDEO =5;	/*4 - YouTube video content download (GET) 	*/
		public static final int HTTP_GOOGLEVIDEO =6;	/*5 - GoogleVideo video content download (GET) */
		public static final int HTTP_VIMEO =7;			/*6 - Vimeo video content download (GET) 	*/
		public static final int HTTP_WIKI =8;			/*7 - Wikipedia (GET) 			*/
		public static final int HTTP_RAPIDSHARE =9;		/*8 - RapidShare file download (GET) 		*/
		public static final int HTTP_MEGAUPLOAD =10;	/*9 - MegaUpload file download (GET) 		*/
		public static final int HTTP_FACEBOOK =11;		/* 10 - Facebook-related connections (GET/POST) */
		public static final int HTTP_ADV =12;			/* 11 - Site advertisement (GET) 		*/
		public static final int HTTP_FLICKR =13;		/* 12 - Flickr photo download (GET) 		*/
		public static final int HTTP_GMAPS =14;			/* 13 - GoogleMaps images (GET) 		*/
		public static final int HTTP_VOD =15; 			/* 14 - Video-On-Demand (GET) [internal use only] */
		public static final int HTTP_YOUTUBE_SITE =16;	/* 15 - YouTube site content download (GET) 	*/
		public static final int HTTP_SOCIAL =17;		/* 16 - Localized social-networking connections */
		 												/*Nasza-Klasa (PL) =; IWIW (HU) (GET/POST) 	*/
		public static final int HTTP_FLASHVIDEO =18;	/* 17 - Generic FLV video download (GET) 	*/
		public static final int HTTP_MEDIAFIRE =19; 	/* 18 - MediaFire file download (GET) 		*/
		public static final int HTTP_HOTFILE =20;		/* 19 - Hotfile.com file download (GET) 	*/
		public static final int HTTP_STORAGE =21; 		/* 20 - Storage.to file download (GET) 		*/
		public static final int HTTP_LAST_TYPE = 22;
	}
	
	public static final class WEB_CATEGORY
	{
		public static final int WEB_GET =1;				/* 0 - Unclassified GET command 			*/
		public static final int WEB_POST =2;			/* 1 - Unclassified POST command 			*/
		public static final int WEB_STORAGE =3; 		/* 2 - Rapidshare =; Megaupload =; Mediafire		*/
		public static final int WEB_YOUTUBE =4;			/* 3 - YouTube only video 			*/
		public static final int WEB_VIDEO =5;			/* 4 - Other Video services 			*/
		public static final int WEB_SOCIAL =6; 			/* 5 - Facebook =; and other social networking 	*/ 
		public static final int WEB_OTHER =7;			/* 6 - All other identified traffic 		*/ 
		public static final int WEB_LAST_TYPE = 8;
	}
	
	public static final class UDP_APP_TYPE{
		public static final int UDP_UNKNOWN = 0;
		public static final int FIRST_RTP = 1;
		public static final int FIRST_RTCP = 2;
		public static final int RTP = 3;
		public static final int RTCP = 4;
		public static final int SKYPE_E2E = 5;
		public static final int SKYPE_OUT = 6;
		public static final int SKYPE_SIG = 7;
		public static final int P2P_EDK = 8;
		public static final int P2P_KAD = 9;
		public static final int P2P_KADU = 10;
		public static final int P2P_GNU = 11;
		public static final int P2P_BT = 12;
		public static final int P2P_DC = 13;
		public static final int P2P_KAZAA = 14;
		public static final int P2P_PPLIVE = 15;
		public static final int P2P_SOPCASE = 16;
		public static final int P2P_TVANTS = 17;
		public static final int P2P_OKAD = 18;
		public static final int DNS = 19;
		public static final int NBNS = 20;
		public static final int LAST_UDP_PROTOCOL = 21;
	}
